...
function Avatar({ name, avatar, size = 'sm' }: { name?: string; avatar?: string | null; size?: 'xs' | 'sm' | 'md' }) {
  const s = { xs: 'w-6 h-6 text-[9px]', sm: 'w-7 h-7 text-[10px]', md: 'w-8 h-8 text-[11px]' }[size]
      ...
      <div className="mt-1.5 flex flex-wrap gap-1.5">
        {(input ? filtered : existingTags).map((tag) => {
          ...
          return (
            <button key={tag.id} type="button"
              ...
              className={cn('px-2 py-0.5 rounded-full text-[11px] font-bold transition-all border', sel ? 'text-white border-transparent' : 'bg-white text-[#6b7280] border-[#e5e7eb] hover:border-[#4648d4]/40')}
        ...
        {input && !exact && (
          <button type="button" onClick={createAndAdd} disabled={creating}
            className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-[#ede9fe] text-[#4648d4] border border-[#4648d4]/20 hover:bg-[#4648d4] hover:text-white transition-all disabled:opacity-50"
          >
...
export default function InitiativeDetailPage() {
  ...
  if (!initiative) {
    return <AppLayout><div className="flex items-center justify-center min-h-screen bg-[#f9fafb]"><p className="text-[#6b7280] text-[13px]">Initiative not found.</p></div></AppLayout>
  }
  ...
  return (
    <AppLayout>
      <div className="bg-[#f9fafb] min-h-screen">
        {/* Compact page header */}
        <div className="bg-white border-b border-[#f0f0f0] px-4 md:px-6 py-4">
          {/* Breadcrumb */}
          <nav className="flex items-center gap-1.5 text-[11px] text-[#9ca3af] mb-2">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between md:gap-4">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className={cn('px-2 py-0.5 text-[10px] font-bold rounded-full', INIT_STATUS[initiative.status]?.cls || INIT_STATUS.active.cls)}>
                  {INIT_STATUS[initiative.status]?.text || initiative.status}
                </span>
                {daysRemaining !== null && (
                  <span className={cn('px-2 py-0.5 text-[10px] font-bold rounded-full', daysRemaining < 0 ? 'bg-[#fef2f2] text-[#dc2626]' : 'bg-[#f2f4f6] text-[#6b7280]')}>
              ...
              <div className="flex items-center gap-3 mt-2.5">
                ...
                <span className="text-[11px] font-semibold text-[#6b7280] tabular-nums">{initiative.progress || 0}%</span>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex -space-x-2">
                      ...
                      {overflow > 0 && (
                        <div className="w-7 h-7 rounded-full bg-[#f2f4f6] text-[#6b7280] text-[9px] font-bold flex items-center justify-center ring-2 ring-white">
                          +{overflow}
                        </div>
                      )}
                    </div>
                ...
                {showUploadDropdown && (
                  <div className="absolute top-full right-0 mt-1 w-48 bg-white rounded-xl shadow-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] z-50 overflow-hidden">
                    {[
                      ...
                      >
                        <span className="material-symbols-outlined text-[16px] text-[#9ca3af]">{icon}</span>{label}
                      </button>
                    ))}
                  </div>
                )}
        ...
        {/* Content */}
        <div className="p-4 md:p-6 grid grid-cols-12 gap-5">
          {/* LEFT — Actions */}
          <section className="col-span-12 lg:col-span-8 space-y-3">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:flex-wrap">
              ...
              <div ref={searchRef} className="relative flex-1 min-w-[180px]">
                <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[#9ca3af] text-[15px]">search</span>
                <input
                  className="w-full pl-8 pr-3 py-1.5 bg-white border border-[#e5e7eb] rounded-lg text-[12px] focus:outline-none focus:ring-2 focus:ring-[#4648d4]/10 focus:border-[#4648d4] transition-all placeholder:text-[#c4c4c4]"
                      ...
                      {searchSuggestions.map((s, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            >{iconMap[s.type]}</span>
                            <span className="text-[12px] font-medium text-[#111827] truncate flex-1">{s.label}</span>
                            {s.sublabel && <span className="text-[11px] text-[#9ca3af] shrink-0">{s.sublabel}</span>}
                            <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-md bg-[#f3f4f6] text-[#6b7280] shrink-0 capitalize">{s.type}</span>
                          </button>
                        )
                      })}
                    </div>
                    <div className="border-t border-[#f3f4f6] px-3 py-1.5">
                      <p className="text-[10px] text-[#c4c4c4]">Press Enter to search · Esc to clear</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex bg-[#f3f4f6] rounded-lg p-0.5 gap-0.5 overflow-x-auto shrink-0">
                {(['all', 'open', 'overdue', 'completed'] as ActionFilter[]).map((f) => (
                  <button key={f} onClick={() => setActionFilter(f)}
                    className={cn('px-2.5 py-1 text-[11px] font-semibold rounded-md transition-all capitalize shrink-0', actionFilter === f
            ...
            {filteredActions.length === 0 ? (
              <div className="bg-white rounded-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] py-12 text-center">
                <span
                ...
                <p className="text-[12px] text-[#9ca3af] font-medium">No actions in this view</p>
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 flex items-start gap-3 px-4 py-3.5 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {action.description && (
                            <p className="text-[11px] text-[#9ca3af] line-clamp-1 mb-1.5">{action.description}</p>
                          )}

                          {actionTags.length > 0 && (
                            <div className="flex gap-1 mt-1.5 flex-wrap">
                              {actionTags.map((tag) => (
                                <span key={tag.id} className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-[#f3f4f6] text-[#6b7280]">
                                  #{tag.name}
                                </span>
                              ))}
                            </div>
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              {action.assignee ? (
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="text-[11px] text-[#6b7280]">{action.assignee.name}</span>
                                </div>
                              ) : (
                                ...
                                confirmDeleteActionId === action.id ? (
                                  <div className="flex items-center gap-1.5">
                                    <button
                                      ...
                                      className="px-2 py-0.5 text-[10px] font-semibold bg-[#dc2626] text-white rounded-md hover:bg-[#b91c1c] transition-colors"
                                    >
                                      Delete
                                    </button>
                                    <button
                                      onClick={(e) => { e.stopPropagation(); setConfirmDeleteActionId(null) }}
                                      className="px-2 py-0.5 text-[10px] font-semibold bg-[#f3f4f6] text-[#6b7280] rounded-md hover:bg-[#e5e7eb] transition-colors"
                                    >
                                      Cancel
                                    </button>
                                  </div>
            ...
            <button onClick={() => setAiMode(true)}
              className="w-full h-10 border border-dashed border-[#e5e7eb] rounded-xl text-[#9ca3af] text-[12px] font-semibold hover:border-[#4648d4]/40 hover:text-[#4648d4] hover:bg-[#f5f3ff]/50 transition-all flex items-center justify-center gap-2 bg-white"
            >
          ...
          <section className="col-span-12 lg:col-span-4 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-4">
              {(searchQuery || actionFilter !== 'all' || tagFilter) && (
                <p className="text-[10px] font-semibold text-[#9ca3af] uppercase tracking-widest mb-3">Filtered view</p>
              )}
              <div className="grid grid-cols-3 gap-3 text-center">
                <div>
                  <p className="text-[20px] font-bold text-[#111827] tabular-nums leading-none">{filteredActions.length}</p>
                  <p className="text-[10px] text-[#9ca3af] uppercase tracking-widest font-semibold mt-1.5">Total</p>
                </div>
                <div>
                  <p className="text-[20px] font-bold text-[#4648d4] tabular-nums leading-none">{filteredActions.filter((a) => a.status !== 'completed').length}</p>
                  <p className="text-[10px] text-[#9ca3af] uppercase tracking-widest font-semibold mt-1.5">Open</p>
                </div>
                <div>
                  {(() => { const od = filteredActions.filter((a) => a.dueDate && isBefore(new Date(a.dueDate), now) && a.status !== 'completed').length; return (
                    <p className={cn('text-[20px] font-bold tabular-nums leading-none', od > 0 ? 'text-[#dc2626]' : 'text-[#111827]')}>{od}</p>
                  )})()}
                  <p className="text-[10px] text-[#9ca3af] uppercase tracking-widest font-semibold mt-1.5">Overdue</p>
                </div>
              </div>
              <div className="mt-4">
                <div className="h-[3px] bg-[#f3f4f6] rounded-full overflow-hidden">
            ...
            {initiative.description && (
              <div className="bg-white rounded-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-4">
                <h3 className="text-[11px] font-bold text-[#9ca3af] uppercase tracking-widest mb-2">About</h3>
            ...
            <div className="bg-white rounded-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] overflow-hidden">
              <div className="px-4 py-3 border-b border-[#f2f4f6] flex items-center justify-between">
                <h3 className="text-[11px] font-bold text-[#9ca3af] uppercase tracking-widest">Recent Activity</h3>
                <span className="text-[10px] text-[#c4c4c4]">{allActions.length} action{allActions.length !== 1 ? 's' : ''}</span>
              </div>
              {allActions.length === 0 ? (
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 pt-0.5">
                            <p className="text-[11px] text-[#111827] leading-snug group-hover:text-[#4648d4] transition-colors">
                            ...
                            <p className="text-[11px] font-medium text-[#374151] line-clamp-1 mt-0.5 group-hover:text-[#4648d4] transition-colors">{action.title}</p>
                            {isOverdue && (
                              <span className="inline-flex items-center gap-0.5 text-[9px] font-semibold text-[#dc2626] mt-0.5">
            ...
            <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: '1px solid #f3f4f6' }}>
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-lg bg-[#ede9fe] flex items-center justify-center">
                  <span className="material-symbols-outlined text-[#4648d4] text-[15px]" style={{ fontVariationSettings: "'FILL' 1" }}>add_task</span>
                </div>
                <div>
                  <h2 className="text-[14px] font-semibold text-[#111827] leading-none">New Action</h2>
                  <p className="text-[11px] text-[#9ca3af] mt-0.5">{initiative.title}</p>
                </div>
              </div>
              <button
              ...
              <div className="p-6 space-y-5 flex-1">
                ...
                <div className="space-y-1">
                    ...
                    <div className="flex gap-1 flex-1">
                      {(['low', 'medium', 'high', 'urgent'] as const).map((p) => (
                        <button key={p} type="button"
                          ...
                          className={cn('px-2 py-0.5 rounded-md text-[11px] font-semibold capitalize transition-all border', actionForm.priority === p
                  ...
                  <div className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-[#f9fafb]">
                    ...
                    <span className="text-[11px] font-medium text-[#9ca3af] w-20 shrink-0">Assignee</span>
                        ...
                        return (
                          <>
                            <button
                              ...
                              className="flex items-center gap-2 text-[12px] font-medium text-[#374151] hover:text-[#4648d4] transition-colors w-full"
                            >
                              {selected ? (
                                <>
                                  <Avatar name={selected.name} avatar={selected.avatar} size="xs" />
                                  <span>{selected.name}</span>
                                  {selected.dept && <span className="ml-1 text-[10px] text-[#9ca3af]">· {selected.dept}</span>}
                                </>
                            ...
                            {showAssigneeDropdown && assigneeBtnRect && createPortal(
                              ...
                              >
                                <button onMouseDown={(e) => { e.preventDefault(); setActionForm((f) => ({ ...f, assigneeId: '' })); setShowAssigneeDropdown(false) }}
                                  className={cn('w-full flex items-center gap-2.5 px-3 py-2 text-[12px] hover:bg-[#f9fafb] transition-colors', !actionForm.assigneeId ? 'text-[#4648d4] font-semibold' : 'text-[#9ca3af]')}
                                >
                                ...
                                {allAssignees.map((a) => (
                                  <button key={a.id} onMouseDown={(e) => { e.preventDefault(); setActionForm((f) => ({ ...f, assigneeId: a.id })); setShowAssigneeDropdown(false) }}
                                    className={cn('w-full flex items-center gap-2.5 px-3 py-2 text-[12px] hover:bg-[#f9fafb] transition-colors', actionForm.assigneeId === a.id ? 'bg-[#f5f3ff]' : '')}
                                  >
                                    ...
                                    {a.role === 'owner' && <span className="text-[9px] font-bold text-[#9ca3af] uppercase shrink-0">owner</span>}
                                  </button>
                                ))}
                              </div>,
                              document.body
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="material-symbols-outlined text-[15px] text-[#9ca3af]">label</span>
                    <span className="text-[11px] font-medium text-[#9ca3af]">Tags</span>
                  </div>
                  <InlineTagInput
      ...
      {showSettings && (
        ...
        >
          <div className="bg-white w-full md:w-[400px] h-full shadow-2xl flex flex-col pt-14 md:pt-0" style={{ borderLeft: '1px solid #f0f0f0' }}>
            ...
            <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: '1px solid #f3f4f6' }}>
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-lg bg-[#f3f4f6] flex items-center justify-center">
                  <span className="material-symbols-outlined text-[#6b7280] text-[15px]">settings</span>
                </div>
                <div>
                  <h2 className="text-[14px] font-semibold text-[#111827] leading-none">Settings</h2>
                  <p className="text-[11px] text-[#9ca3af] mt-0.5 truncate max-w-[220px]">{initiative.title}</p>
                </div>
              </div>
            ...
            {/* Tabs */}
            <div className="flex px-6" style={{ borderBottom: '1px solid #f3f4f6' }}>
              {(['members', 'notifications'] as SettingsTab[]).map((tab) => (
                <button key={tab} onClick={() => setSettingsTab(tab)}
                  className={cn('py-3 px-1 mr-5 text-[11px] font-semibold border-b-2 -mb-px transition-all capitalize', settingsTab === tab ? 'border-[#4648d4] text-[#4648d4]' : 'border-transparent text-[#9ca3af] hover:text-[#6b7280]')}
                >
                  {tab}
                </button>
              ))}
                ...
                <div>
                  ...
                  <div className="divide-y divide-[#f9fafb]">
                    ...
                    {(initiative.pending ?? []).map((p) => (
                      <div key={p.id} className="flex items-center gap-3 px-6 py-3.5 hover:bg-[#fafafa] transition-colors opacity-60">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[12px] font-semibold text-[#111827] truncate">{p.email}</p>
                          <p className="text-[10px] text-[#9ca3af]">Hasn't signed in yet</p>
                        </div>
                        <span className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full bg-[#fef9c3] text-[#a16207]">Invited</span>
                      </div>
                    ))}
                  </div>
                  ...
                  {isOwnerOrAdmin && <div className="px-6 py-5" style={{ borderTop: '1px solid #f3f4f6' }}>
                    ...
                    <form onSubmit={handleInvite} className="space-y-3">
                      <input
                        ...
                        className="w-full h-9 px-3 bg-white border border-[#e5e7eb] rounded-lg text-[12px] text-[#111827] focus:ring-2 focus:ring-[#4648d4]/10 focus:border-[#4648d4] focus:outline-none transition-all placeholder:text-[#c4c4c4]"
                      />
                      {/* Role pills */}
                      <div>
                        <p className="text-[10px] font-semibold text-[#9ca3af] uppercase tracking-widest mb-1.5">Role</p>
                        <div className="flex gap-2">
                          {(['member', 'admin'] as const).map((r) => (
                            <button key={r} type="button" onClick={() => setInviteRole(r)}
                              className={cn('flex-1 h-8 text-[11px] font-semibold rounded-lg capitalize border transition-all', inviteRole === r ? 'bg-[#4648d4] text-white border-[#4648d4]' : 'bg-white text-[#6b7280] border-[#e5e7eb] hover:border-[#4648d4]/30')}
                            >{r}</button>
                          ))}
                        </div>
                      </div>
                      {/* Department */}
                      <div>
                        <p className="text-[10px] font-semibold text-[#9ca3af] uppercase tracking-widest mb-1.5">Department / Function</p>
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          {['Engineering', 'Sales', 'Presales', 'Consultant', 'Solution', 'Marketing', 'Finance'].map((d) => (
                            <button key={d} type="button" onClick={() => setInviteDepartment(inviteDepartment === d ? '' : d)}
                              className={cn('px-2.5 py-1 rounded-full text-[11px] font-medium border transition-all', inviteDepartment === d ? 'bg-[#ede9fe] text-[#4648d4] border-[#c4b5fd]' : 'bg-white text-[#6b7280] border-[#e5e7eb] hover:border-[#4648d4]/30')}
                            >{d}</button>
                          ))}
                        </div>
                        <input
                          ...
                          className="w-full h-8 px-3 bg-white border border-[#e5e7eb] rounded-lg text-[12px] text-[#111827] focus:ring-2 focus:ring-[#4648d4]/10 focus:border-[#4648d4] focus:outline-none transition-all placeholder:text-[#c4c4c4]"
                        />
                      </div>
                      <button
                        type="submit"
                        disabled={inviting || !inviteEmail.trim()}
                        className="w-full h-9 bg-[#4648d4] text-white rounded-lg text-[12px] font-semibold hover:bg-[#3730a3] transition-colors disabled:opacity-40"
                      >
                  ...
                  {notifSettings.dailyReportEnabled && (
                    <div className="pt-1">
                      ...
                      <input type="time" value={notifSettings.dailyReportTime}
                        ...
                        className="w-full h-9 px-3 bg-white border border-[#e5e7eb] rounded-lg text-[13px] text-[#111827] focus:ring-2 focus:ring-[#4648d4]/10 focus:border-[#4648d4] focus:outline-none transition-all"
                      />
                      <p className="text-[11px] text-[#9ca3af] mt-1.5">Digest will be sent to all initiative members.</p>
                    </div>
                  )}
        ...
        >
          <div className="bg-white w-full md:w-[500px] h-full shadow-2xl flex flex-col pt-14 md:pt-0" style={{ borderLeft: '1px solid #f0f0f0' }}>
            ...
            <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: '1px solid #f3f4f6' }}>
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-lg bg-[#ede9fe] flex items-center justify-center">
                  <span className="material-symbols-outlined text-[#4648d4] text-[15px]" style={{ fontVariationSettings: "'FILL' 1" }}>auto_awesome</span>
                </div>
                <div>
                  <h2 className="text-[14px] font-semibold text-[#111827] leading-none">AI Action Generator</h2>
                  <p className="text-[11px] text-[#9ca3af] mt-0.5">Paste transcript or meeting notes</p>
                </div>
              </div>
              ...
              {generatedActions.length === 0 ? (
                <div className="p-6 flex flex-col gap-4 flex-1">
                  <textarea rows={14} placeholder="Paste your meeting transcript, notes, or voice recording text here...&#10;&#10;AI will extract action items, assign them to team members by name or department, and detect priorities and deadlines."
                    ...
                    className="w-full flex-1 bg-[#f9fafb] border border-[#e5e7eb] rounded-xl px-4 py-3 text-[13px] text-[#111827] focus:ring-2 focus:ring-[#4648d4]/10 focus:border-[#4648d4] focus:outline-none resize-none placeholder:text-[#9ca3af] leading-relaxed"
                  />
                  <button onClick={handleGenerateActions} disabled={generating || !transcript.trim()}
                    className="w-full h-10 bg-[#4648d4] text-white font-semibold rounded-xl hover:bg-[#3730a3] transition-colors disabled:opacity-40 flex items-center justify-center gap-2 text-[13px]"
                  >
                    {generating
                      ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /><span>Extracting actions...</span></>
                      : <><span className="material-symbols-outlined text-[16px]">auto_awesome</span><span>Generate Actions</span></>
                    }
                  </button>
                </div>
              ) : (
                <>
                  <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <button onClick={() => { setGeneratedActions([]); setEditingAiIndex(null) }} className="text-[11px] font-semibold text-[#9ca3af] hover:text-[#4648d4] transition-colors">Re-generate</button>
                  </div>
                          ...
                          {isEditing ? (
                            <div className="space-y-3">
                              <input
                                ...
                                className="w-full text-[13px] font-semibold text-[#111827] focus:outline-none border-b border-[#4648d4]/30 pb-1 bg-transparent"
                              />
                              <textarea rows={2}
                                value={action.description || ''}
                                onChange={(e) => setGeneratedActions((acts) => acts.map((a, idx) => idx === i ? { ...a, description: e.target.value } : a))}
                                placeholder="Description..."
                                className="w-full text-[12px] text-[#374151] focus:outline-none bg-[#f9fafb] border border-[#e5e7eb] rounded-lg px-3 py-2 resize-none placeholder:text-[#9ca3af]"
                              />
                              {/* Priority pills */}
                              <div className="flex gap-1.5">
                                {(['low', 'medium', 'high', 'urgent'] as const).map((p) => (
                                  <button key={p} type="button"
                                    ...
                                    className={cn('px-2 py-0.5 rounded-md text-[11px] font-semibold capitalize border transition-all', action.priority === p
                                      ? p === 'urgent' ? 'bg-[#fef2f2] text-[#dc2626] border-[#fecaca]'
                                        : p === 'high' ? 'bg-[#ede9fe] text-[#4648d4] border-[#c4b5fd]'
                                        : p === 'medium' ? 'bg-[#eff6ff] text-[#2563eb] border-[#bfdbfe]'
                                        : 'bg-[#f3f4f6] text-[#6b7280] border-[#e5e7eb]'
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <select
                                  ...
                                  className="flex-1 h-8 px-2 bg-white border border-[#e5e7eb] rounded-lg text-[12px] text-[#374151] focus:outline-none focus:border-[#4648d4] cursor-pointer"
                                >
                              ...
                              {/* Due date */}
                              <div className="flex items-center gap-2">
                                <span className="material-symbols-outlined text-[14px] text-[#9ca3af]">event</span>
                                <input type="date"
                                  ...
                                  className="h-8 px-2 bg-white border border-[#e5e7eb] rounded-lg text-[12px] text-[#374151] focus:outline-none focus:border-[#4648d4]"
                                />
                              </div>
                              <button type="button" onClick={() => setEditingAiIndex(null)}
                                className="text-[11px] font-semibold text-[#4648d4] hover:text-[#3730a3]">Done editing</button>
                            </div>
                          ) : (
                            <div className="flex items-start gap-3 group cursor-pointer" onClick={() => setEditingAiIndex(i)}>
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-[13px] font-semibold text-[#111827] leading-snug">{action.title}</p>
                                {action.description && <p className="text-[11px] text-[#9ca3af] mt-0.5 line-clamp-2">{action.description}</p>}
                                <div className="flex items-center gap-3 mt-1.5">
                                  {assignedPerson && (
                                    <div className="flex items-center gap-1">
                                      <Avatar name={assignedPerson.name} avatar={assignedPerson.avatar} size="xs" />
                                      <span className="text-[11px] text-[#6b7280]">{assignedPerson.name}</span>
                                    </div>
                                  )}
                                    {action.dueDate && <span className="text-[11px] text-[#9ca3af]">{format(new Date(action.dueDate), 'MMM d')}</span>}
                                </div>
                                {action.tags?.length > 0 && (
                                  <div className="flex gap-1 flex-wrap mt-1">
                                    {action.tags.map((tag: string, ti: number) => (
                                      <span key={ti} className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-[#f3f4f6] text-[#6b7280]">#{tag}</span>
                                    ))}
                                  </div>
                                )}
                              </div>
                              <span className="material-symbols-outlined text-[14px] text-[#d1d5db] group-hover:text-[#9ca3af] transition-colors shrink-0">edit</span>
                            </div>
                          )}
                        </div>
                      )
                  ...
                  <div className="px-6 py-4 flex gap-2.5" style={{ borderTop: '1px solid #f3f4f6' }}>
                    <button onClick={handleBulkSave} disabled={bulkSaving}
                      className="flex-1 h-9 bg-[#4648d4] text-white text-[12px] font-semibold rounded-lg hover:bg-[#3730a3] transition-colors disabled:opacity-40"
                    >
