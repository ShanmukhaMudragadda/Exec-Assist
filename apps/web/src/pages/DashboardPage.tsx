...
export default function DashboardPage() {
  ...
  return (
    <AppLayout>
      <div className="min-h-screen p-4 md:p-7 max-w-[1200px]">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-5 mt-1 flex-wrap">
            <div className="text-right">
              ...
              <p className="text-[10px] text-[#9ca3af] uppercase tracking-wider font-semibold mt-1">Open</p>
            </div>
            {overdueActions.length > 0 && (
              <div className="text-right">
                <p className="text-[22px] font-bold text-[#dc2626] tabular-nums leading-none">{overdueActions.length}</p>
                <p className="text-[10px] text-[#dc2626] uppercase tracking-wider font-semibold mt-1">Overdue</p>
              </div>
            )}
            <div className="text-right">
              <p className="text-[22px] font-bold text-[#4648d4] tabular-nums leading-none">{allInitiatives.length}</p>
              <p className="text-[10px] text-[#9ca3af] uppercase tracking-wider font-semibold mt-1">Initiatives</p>
            </div>
          </div>
        </div>

        {/* Executive Brief — newspaper editorial */}
        <div className="mb-8 overflow-hidden" style={{ background: '#ffffff', border: '1px solid #e2e8f0', boxShadow: '0 4px 16px rgba(0,0,0,0.07)' }}>
          {/* Masthead */}
          <div className="flex items-center justify-between px-6 py-3">
            <div className="flex items-center gap-2.5">
              <span className="material-symbols-outlined text-[13px] text-[#4648d4]" style={{ fontVariationSettings: "'FILL' 1" }}>auto_awesome</span>
              <span className="font-serif text-[16px] font-bold text-[#111827] tracking-tight">Executive Brief</span>
              <span className="text-[7px] font-bold px-1.5 py-0.5 bg-[#4648d4] text-white tracking-widest uppercase">AI</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-[10px] text-[#9ca3af]">{format(now, 'EEEE, MMMM d, yyyy')}</span>
              <button
              ...
              <Link to="/command-center" className="text-[10px] font-semibold text-[#4648d4] hover:opacity-70 transition-opacity">
                View all →
              </Link>
            </div>
          </div>
          ...
          {/* Body */}
          {briefLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-0 px-6 py-2">
              {[0, 1].map((col) => (
                <div key={col} className={cn('divide-y', col === 1 && 'pl-6')} style={{ borderColor: '#e2e8f0' }}>
                  {[0, 1].map((row) => (
                    <div key={row} className="py-5 animate-pulse space-y-2">
            ...
            (() => {
              ...
              const renderItem = (item: typeof aiBriefPoints[0], idx: number) => {
                ...
                return (
                  <div key={idx} className="py-5" style={{ borderBottom: '1px solid #e2e8f0' }}>
                    <div className="flex items-center gap-1.5 mb-2">
                      <div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: color }} />
                      <span className="text-[8.5px] font-bold uppercase tracking-[0.14em]" style={{ color }}>{label}</span>
                    </div>
                    <h3 className="font-serif text-[20px] font-bold leading-tight mb-2.5" style={{ color: '#0f172a' }}>
                      {item.headline}
                    </h3>
                    <p className="text-[12.5px] leading-[1.75]" style={{ color: '#64748b' }}>
                      {item.detail}
                    </p>
                  </div>
                )
            ...
            <div className="py-10 text-center px-6">
              <p className="text-[12px] mb-3" style={{ color: '#9ca3af' }}>Brief not available.</p>
              <button onClick={() => refetchBrief()} className="text-[11px] font-semibold text-[#4648d4] hover:opacity-70 transition-opacity">
                Generate Brief →
              </button>
            </div>
          )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="col-span-12 lg:col-span-8 space-y-5">
            ...
            <div className="bg-white rounded-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#f9fafb]">
                <div className="flex items-center gap-2.5">
                  <h2 className="text-[13px] font-semibold text-[#111827]">Priority Queue</h2>
                  {overdueActions.length > 0 && (
                ...
                <Link to="/command-center" className="text-[11px] font-medium text-[#9ca3af] hover:text-[#4648d4] transition-colors">
                  See all
                </Link>
              </div>
              ...
              {priorityFeed.length === 0 ? (
                <div className="py-12 text-center">
                  ...
                  <p className="text-[12px] text-[#9ca3af]">All clear — no priority actions.</p>
                </div>
              ) : (
                  ...
                  {priorityFeed.map((action) => {
                    ...
                    return (
                      <div
                        ...
                        className="flex items-center gap-3 px-5 py-3 hover:bg-[#fafafa] cursor-pointer group transition-colors duration-100"
                      >
                        <div
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-medium text-[#111827] truncate group-hover:text-[#4648d4] transition-colors">
                            {action.title}
                          </p>
                          <p className="text-[11px] text-[#9ca3af] truncate mt-0.5">{action.initiative?.title}</p>
                        </div>
                        <div className="flex items-center gap-2.5 shrink-0">
                          ...
                          {assigneeInitials && (
                            <div className="w-5 h-5 rounded-full bg-[#ede9fe] text-[#4648d4] text-[9px] font-bold flex items-center justify-center">
                              {assigneeInitials}
                            </div>
                          )}
                        </div>
            ...
            <div className="bg-white rounded-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#f9fafb]">
                <h2 className="text-[13px] font-semibold text-[#111827]">Initiative Pulse</h2>
                <Link to="/initiatives" className="text-[11px] font-medium text-[#9ca3af] hover:text-[#4648d4] transition-colors">
                  See all
                </Link>
              </div>
              ...
              {allInitiatives.length === 0 ? (
                <div className="py-12 text-center">
                  <p className="text-[12px] text-[#9ca3af]">No initiatives yet.</p>
                  <Link to="/initiatives" className="text-[12px] font-semibold text-[#4648d4] hover:opacity-70 mt-1 inline-block transition-opacity">
                    Create one →
                  </Link>
                </div>
              ) : (
                  ...
                  {allInitiatives.slice(0, 6).map((init) => {
                    ...
                    return (
                      <div
                        ...
                        className="flex items-center gap-4 px-5 py-3.5 hover:bg-[#fafafa] cursor-pointer group transition-colors duration-100"
                      >
                        <div
                        ...
                        <div className="flex-1 min-w-0 space-y-1.5">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              {isAtRisk && (
                                <span className="text-[10px] font-semibold text-[#dc2626] bg-[#fef2f2] px-1.5 py-0.5 rounded-full">At Risk</span>
                              )}
                              <span className="text-[11px] font-semibold tabular-nums text-[#6b7280]">{init.progress || 0}%</span>
                            </div>
                          </div>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-5">
              <p className="text-[10px] font-semibold text-[#9ca3af] uppercase tracking-widest mb-4">Today's Progress</p>
              <div className="grid grid-cols-2 gap-3">
                <div
                  className="rounded-xl p-3.5 text-center"
                  style={{ background: 'linear-gradient(135deg, #f9fafb 0%, #f3f4f6 100%)' }}
                >
                  <p className="text-[24px] font-bold text-[#111827] tabular-nums leading-none">{completedActions.length}</p>
                  <p className="text-[10px] text-[#9ca3af] uppercase tracking-wider font-semibold mt-2">Completed</p>
                </div>
                <div
                  className="rounded-xl p-3.5 text-center"
                  style={{ background: 'linear-gradient(135deg, #f5f3ff 0%, #ede9fe 100%)' }}
                >
                  <p className="text-[24px] font-bold text-[#4648d4] tabular-nums leading-none">{todayActions.length}</p>
                  <p className="text-[10px] text-[#9ca3af] uppercase tracking-wider font-semibold mt-2">Due Today</p>
                </div>
              </div>
            </div>
            ...
            {/* Recent Actions */}
            <div className="bg-white rounded-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] overflow-hidden">
              <div className="px-5 py-3.5 border-b border-[#f9fafb]">
                <p className="text-[10px] font-semibold text-[#9ca3af] uppercase tracking-widest">Recent Actions</p>
              </div>
              <div className="divide-y divide-[#fafafa]">
                {allActions.slice(0, 6).map((action) => (
                  <div
                    key={action.id}
                    onClick={() => navigate(getActionPath(action))}
                    className="flex items-center gap-3 px-5 py-2.5 hover:bg-[#fafafa] cursor-pointer group transition-colors duration-100"
                  >
                    <div
                      className="w-1.5 h-1.5 rounded-full shrink-0"
                      style={{
                    ...
                    />
                    <p className="text-[12px] font-medium text-[#374151] line-clamp-1 group-hover:text-[#4648d4] transition-colors">
                      {action.title}
                    </p>
                  </div>
                ))}
                {allActions.length === 0 && (
                  <div className="px-5 py-8 text-center text-[12px] text-[#9ca3af]">No actions yet.</div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-[#f0f0f0] shadow-[0_1px_4px_rgba(0,0,0,0.04)] p-4">
              <p className="text-[10px] font-semibold text-[#9ca3af] uppercase tracking-widest mb-3">Quick Access</p>
              <div className="space-y-0.5">
                {[
                  ...
                  >
                    <span className="material-symbols-outlined text-[16px] text-[#9ca3af] group-hover:text-[#4648d4] transition-colors">
                      {icon}
                    </span>
                    <span className="text-[12px] font-medium text-[#374151] group-hover:text-[#4648d4] transition-colors flex-1">
                      {label}
                    </span>
                    <span className="material-symbols-outlined text-[14px] text-[#e5e7eb] group-hover:text-[#4648d4] transition-colors">
                      arrow_forward
                    </span>
                  </Link>
                ))}
