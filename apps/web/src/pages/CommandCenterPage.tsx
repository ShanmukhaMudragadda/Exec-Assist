...
export default function CommandCenterPage() {
  ...
  return (
    <AppLayout>
      <div className="min-h-screen p-4 md:p-7 max-w-[900px]">
        {/* Header */}
        <div className="mb-7">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div ref={dropdownRef} className="relative flex shrink-0">
              <button
                ...
                className="px-3 py-2 bg-[#4648d4] text-white text-[12px] font-bold rounded-l-lg flex items-center gap-1.5 hover:bg-[#3730a3] transition-colors border-r border-[#3730a3]"
              >
        ...
        <div className="flex items-center gap-1.5 mb-6 overflow-x-auto pb-1">
          {filterConfig.map(({ key, label, count }) => (
            <button
              ...
              className={cn(
                'px-3 py-1.5 rounded-lg text-[11px] font-semibold transition-all duration-150 shrink-0',
                filter === key
                  ? key === 'overdue'
            ...
            return (
              <section key={initiative.id}>
                ...
                >
                  ...
                  />
                  <span className={cn('text-[12px] font-semibold', isStandalone ? 'text-[#6b7280]' : 'text-[#4648d4] group-hover:underline')}>
                    {initiative.title}
                  </span>
                  {!isStandalone && initiative.status === 'at-risk' && (
                    <span className="px-1.5 py-0.5 text-[9px] font-semibold rounded-full bg-[#fef2f2] text-[#dc2626]">At Risk</span>
                  )}
                  <span className="text-[11px] text-[#d1d5db] ml-0.5">· {actions.length}</span>
                </div>
                        ...
                        <div className="flex-1 flex items-start gap-3 px-4 py-3.5 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <h4
                              ...
                              className="text-[13px] font-medium text-[#111827] truncate cursor-pointer group-hover:text-[#4648d4] transition-colors"
                            >
                              {action.title}
                            </h4>
                            {action.description && (
                              <p className="text-[11px] text-[#9ca3af] mt-0.5 line-clamp-1">{action.description}</p>
                            )}
                            ...
                            {actionTags.length > 0 && (
                              <div className="flex gap-1 mt-1.5 flex-wrap">
                                {actionTags.map((tag: any) => (
                                  <span
                                    key={tag.id}
                                    className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-[#f3f4f6] text-[#6b7280]"
                                  >
                                    #{tag.name}
                                  </span>
                                ))}
                              ...
                              {action.assignee ? (
                                <div className="flex items-center gap-1.5">
                                  {assigneeAvatar ? (
                                    ...
                                    <div className="w-4 h-4 rounded-full bg-[#ede9fe] text-[#4648d4] text-[8px] font-bold flex items-center justify-center shrink-0">
                                      {assigneeInitials}
                                    </div>
                                  )}
                            ...
                            <div className="flex items-center gap-1.5 mt-2">
                              <button
                                ...
                                className="px-2 py-1 text-[10px] font-semibold text-[#6b7280] hover:bg-[#f3f4f6] rounded-md transition-colors border border-[#e5e7eb] hover:border-[#d1d5db]"
                              >
                                In Progress
                              </button>
                              <button
                                onClick={(e) => { e.stopPropagation(); updateAction(action.id, 'completed') }}
                                className="px-2 py-1 text-[10px] font-semibold text-[#4648d4] hover:bg-[#ede9fe] rounded-md transition-colors"
                              >
                                ✓ Done
                              </button>
                              <button
                                onClick={() => navigate(actionPath)}
                                className="ml-auto text-[10px] font-medium text-[#9ca3af] hover:text-[#4648d4] transition-colors"
                              >
                                View →
                              </button>
                            </div>
          ...
          <div className="bg-white w-full md:w-[440px] h-full shadow-2xl flex flex-col pt-14 md:pt-0" style={{ borderLeft: '1px solid #f0f0f0' }}>
            <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: '1px solid #f3f4f6' }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-[14px] font-semibold text-[#111827] leading-none">AI Generate Actions</h2>
                  <p className="text-[11px] text-[#9ca3af] mt-0.5">Command Center · Standalone</p>
                </div>
              </div>
              ...
              {generatedActions.length === 0 ? (
                ...
                <>
                  <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #f3f4f6' }}>
                    ...
                    <button onClick={() => { setGeneratedActions([]); setEditingIdx(null) }} className="text-[11px] font-semibold text-[#9ca3af] hover:text-[#4648d4] transition-colors">Re-generate</button>
                  </div>
                        ...
                        <div key={i} className="px-6 py-4">
                          {isEditing ? (
                            <div className="space-y-3">
                              ...
                              <div className="flex gap-1.5">
                                {(['low', 'medium', 'high', 'urgent'] as const).map((p) => (
                                  <button key={p} type="button"
                                    ...
                                    className={cn('px-2 py-0.5 rounded-md text-[11px] font-semibold capitalize border transition-all',
                              ...
                              <button type="button" onClick={() => setEditingIdx(null)} className="text-[11px] font-semibold text-[#4648d4] hover:text-[#3730a3]">Done editing</button>
                            </div>
                          ) : (
          ...
          <div className="bg-white w-full md:w-[440px] h-full shadow-2xl flex flex-col pt-14 md:pt-0" style={{ borderLeft: '1px solid #f0f0f0' }}>
            <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: '1px solid #f3f4f6' }}>
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-[14px] font-semibold text-[#111827]">New Action</h2>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              <textarea placeholder="Add a description (optional)..."
                ...
                className="w-full text-[13px] text-[#374151] placeholder:text-[#c4c4c4] focus:outline-none bg-transparent resize-none"
              />
              <div className="flex items-center gap-3">
                <span className="material-symbols-outlined text-[16px] text-[#9ca3af]">flag</span>
                <span className="text-[12px] text-[#6b7280] w-20 shrink-0">Priority</span>
                <div className="flex gap-1.5 flex-wrap">
                  {(['low', 'medium', 'high', 'urgent'] as const).map((p) => (
                    <button key={p} onClick={() => setNewPriority(p)}
                      className={cn('px-2.5 py-1 rounded-lg text-[11px] font-semibold capitalize border transition-all',
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-[12px] text-[#6b7280] w-20 shrink-0">Due Date</span>
                <button onClick={() => newDateRef.current?.showPicker?.()}
                  className="text-[12px] font-medium text-[#111827] hover:text-[#4648d4] transition-colors"
                >
                  {newDueDate ? format(new Date(newDueDate), 'MMM d, yyyy') : <span className="text-[#9ca3af] font-normal">Pick a date</span>}
                </button>
                {newDueDate && <button onClick={() => setNewDueDate('')} className="text-[#9ca3af] hover:text-[#dc2626] text-[14px] leading-none">×</button>}
                <input ref={newDateRef} type="date" value={newDueDate} onChange={(e) => setNewDueDate(e.target.value)} className="sr-only" />
              </div>
              <p className="text-[11px] text-[#9ca3af] bg-[#f9fafb] rounded-lg px-3 py-2">
            ...
            <div className="px-6 py-4 flex gap-3" style={{ borderTop: '1px solid #f0f0f0' }}>
              <button onClick={resetPane} className="flex-1 px-4 py-2.5 border border-[#e5e7eb] text-[13px] font-semibold text-[#6b7280] rounded-xl hover:bg-[#f9fafb] transition-colors">Cancel</button>
              <button onClick={handleCreateAction} disabled={!newTitle.trim() || creating}
                className="flex-1 px-4 py-2.5 bg-[#4648d4] text-white text-[13px] font-semibold rounded-xl hover:bg-[#3730a3] transition-colors disabled:opacity-40"
              >
